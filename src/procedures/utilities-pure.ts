...
export function try_complete_url(url: string): URL | null {
  const attempts = [url, `https://${url}`].map(url => URL.parse(url)).find(value => value !== null);
...
export async function attempts_to<T>(
  ...
  const final_result = Array.from<{ succeed: true; result: any } | { succeed: false; reason: any }>({
    length: callables.length,
  });
  async function try_tasks(
    ...
    for (const {
      ...
      if (result.status !== "fulfilled") {
        continue; // never happens, just here to make TypeScript happy
      }
